...
import { SCENE_STYLES, sceneStyle } from "@/lib/scene-styles";
import { SCENE_PRESETS, sceneFromPreset, type ScenePresetId } from "@/lib/scene-presets";
import CanvasTextEditor from "./CanvasTextEditor";
import SceneThumbnail from "./SceneThumbnail";
import ScenePresetGallery from "./ScenePresetGallery";
...
export default function CanvasStudio({ data, mutate, initialProjectId, initialRecordId, onDirtyChange, onProject, onLegacy, onImport, onLogout }: Props) {
  const approved = useMemo(() => data.assets.filter((asset) => asset.active && asset.rightsStatus === "approved"), [data.assets]);
  ...
  const [savedSignature, setSavedSignature] = useState(() => initial ? signature(editor) : "");
  ...
  const [time, setTime] = useState(() => settleTime(editor.doc, selectedId));
  ...
  const currentSignature = signature(editor);
  ...
  const edit = useCallback((next: Editor | ((current: Editor) => Editor), coalesce = true) => {
    const current = editorRef.current;
    ...
    editorRef.current = updated; setEditor(updated); setSaveError(""); syncHistory();
  }, [syncHistory]);
  ...
  useEffect(() => { saveRef.current = saveNow; });
  useEffect(() => {
  ...
  }, [dirty, currentSignature, saving, busy, assetsReady, saveError]);
  ...
  const switchTo = async (next: Editor, project?: Project) => {
    ...
    history.current = { past: [], future: [], last: 0 }; syncHistory(); setPanel(null); setSaveError(""); setMessage(""); onProject(project?.id || "");
  };
  const openRecord = (record: ContentRecord) => switchTo({ doc: newCanvasDocument(record, record.assetId ? `library-${record.assetId}` : photoId ? `library-${photoId}` : null, data.scheduleRules), title: record.name, mode: editor.mode, plannedDate: "", recordId: record.id });
  const addScene = (presetId: ScenePresetId, presetImageId: string | null) => {
    if (busy || editorRef.current.doc.scenes.length >= 30) return;
    const scene = sceneFromPreset(presetId, presetImageId);
    edit((current) => {
      const scenes = [...current.doc.scenes];
      scenes.splice(scenes.findIndex((item) => item.id === selected.id) + 1, 0, scene);
      return { ...current, doc: { ...current.doc, designVersion: 2, scenes } };
    }, false);
    seek(scene.id); setMessage(`${SCENE_PRESETS.find((preset) => preset.id === presetId)?.name} added. Click its words to make it yours.`);
  };
  const undoRedo = (redo: boolean) => {
    ...
    to.push(editorRef.current); editorRef.current = next; setEditor(next); setSelectedId(next.doc.scenes[0].id); timeRef.current = settleTime(next.doc, next.doc.scenes[0].id); setPlaying(false); setSaveError(""); syncHistory();
  };
  ...
  const galleryPhoto = selected.imageId || photos[0]?.id || null;
  const color = sceneStyle(selected.styleId, selected.template === "endcard");
  ...
  return <div className="canvas-studio">
    ...
    <div className={`cs-workspace ${editor.mode === "graphic" ? "cs-graphic-mode" : ""}`}>
      <aside className="cs-scenes"><div className="cs-panel-title">{editor.mode === "video" ? "Scenes" : "Artboards"}<span>{editor.doc.scenes.length}</span></div>{editor.doc.scenes.map((scene, index) => <button key={scene.id} className={`cs-scene ${scene.id === selected.id ? "active" : ""}`} disabled={busy} onClick={() => seek(scene.id)}><SceneThumbnail doc={editor.doc} scene={scene} images={images} /><span><b>{String(index + 1).padStart(2, "0")}</b>{scene.title || "Untitled"}{editor.mode === "video" && <small>{scene.duration / 1000}s</small>}</span></button>)}<button className="cs-add" onClick={() => { document.getElementById("cs-scene-gallery")?.scrollIntoView({ block: "nearest" }); document.getElementById("cs-first-preset")?.focus(); }}><Plus size={15} />Browse scenes</button></aside>
      <main className="cs-center"><div className="cs-stage"><canvas ref={canvasRef} style={{ aspectRatio: `${MOTION_ASPECTS[editor.doc.aspect].width} / ${MOTION_ASPECTS[editor.doc.aspect].height}` }} aria-label="Sun Oaks live canvas" />{!playing && !busy && editor.doc.designVersion === 2 && <CanvasTextEditor key={selected.id} canvasRef={canvasRef} scene={selected} aspect={editor.doc.aspect} ready={assetsReady} onChange={patchScene} />}</div><div className="cs-transport">{editor.mode === "video" && <><button className="cs-play" aria-label={playing ? "Pause preview" : "Play preview"} onClick={() => { if (playing) seek(motionSceneAt(editor.doc, timeRef.current).scene.id); else { timeRef.current = 0; setPlaying(true); } }}>{playing ? <Pause size={17} /> : <Play size={17} />}</button><span>{(time / 1000).toFixed(1)} / {(total / 1000).toFixed(1)}s</span></>}<button className="cs-icon" disabled={busy || !historyState.canUndo} onClick={() => undoRedo(false)} aria-label="Undo"><Undo2 size={17} /></button><button className="cs-icon" disabled={busy || !historyState.canRedo} onClick={() => undoRedo(true)} aria-label="Redo"><Redo2 size={17} /></button><span className="cs-stage-hint">{editor.mode === "graphic" ? "PNG exports the selected artboard" : "Click any text on the canvas to edit"}</span></div>{editor.mode === "video" && <div className="cs-timeline"><div className="cs-timeline-labels">{editor.doc.scenes.map((scene) => <button key={scene.id} style={{ flex: scene.duration }} onClick={() => seek(scene.id)}>{scene.title || "Scene"}</button>)}</div><input aria-label="Video playhead" type="range" min={0} max={Math.max(1, total - 1)} value={time} onChange={(event) => { const value = Number(event.target.value); timeRef.current = value; setTime(value); setPlaying(false); setSelectedId(motionSceneAt(editor.doc, value).scene.id); }} /></div>}<ScenePresetGallery aspect={editor.doc.aspect} photoId={galleryPhoto} images={images} disabled={busy || editor.doc.scenes.length >= 30} onAdd={addScene} /></main>
      <aside className="cs-inspector"><fieldset disabled={busy}><div className="cs-inspector-head"><strong>Your message</strong><button type="button" className="cs-link" onClick={() => setPanel("content")}>Use class / event</button></div><label>Headline<textarea id="cs-headline" rows={3} maxLength={240} value={selected.title} onChange={(event) => patchScene({ title: event.target.value })} /></label><label>Small line<textarea rows={2} maxLength={400} value={selected.subtitle} onChange={(event) => patchScene({ subtitle: event.target.value })} /></label><label>Small label<input maxLength={160} value={selected.kicker} onChange={(event) => patchScene({ kicker: event.target.value })} /></label><div className="cs-panel-title">Color style<button className="cs-link" onClick={() => { edit((current) => ({ ...current, doc: { ...current.doc, designVersion: 2, scenes: current.doc.scenes.map((scene) => ({ ...scene, styleId: color.id })) } }), false); seek(selected.id); }}>Apply to all</button></div><div className="cs-colors">{SCENE_STYLES.map((style) => <button key={style.id} className={color.id === style.id ? "active" : ""} aria-label={`Color style: ${style.name}`} aria-pressed={color.id === style.id} onClick={() => { if (editor.doc.designVersion !== 2) edit((current) => ({ ...current, doc: { ...current.doc, designVersion: 2 } }), false); patchScene({ styleId: style.id }); }}><span style={{ background: style.background, color: style.ink }}><b>Aa</b><i style={{ background: style.accent }} />{color.id === style.id && <Check size={12} />}</span><small>{style.name}</small></button>)}</div><div className="cs-panel-title">Sun Oaks logo</div><div className="cs-logo-choices"><button aria-pressed={selected.logoStyle === "emblem"} className={selected.logoStyle === "emblem" ? "active" : ""} onClick={() => { if (editor.doc.designVersion !== 2) edit((current) => ({ ...current, doc: { ...current.doc, designVersion: 2 } }), false); patchScene({ logoStyle: "emblem" }); }}><span className="cs-emblem-sample"><img src="/studio/brand/logos/SunOaks_Emblem.svg" alt="" /></span><strong>Oversized oak</strong><small>Cropped emblem</small></button><button aria-pressed={selected.logoStyle !== "emblem"} className={selected.logoStyle !== "emblem" ? "active" : ""} onClick={() => { if (editor.doc.designVersion !== 2) edit((current) => ({ ...current, doc: { ...current.doc, designVersion: 2 } }), false); patchScene({ logoStyle: "wordmark" }); }}><span><img src={SUN_OAKS_LOGO} alt="" /></span><strong>Small wordmark</strong><small>Name + graphic</small></button></div><div className="cs-panel-title">Background<button className="cs-link" onClick={() => fileRef.current?.click()}><ImagePlus size={14} />{uploading ? "Uploading…" : "Upload"}</button></div><div className="cs-photos">{photos.map((photo) => <button key={photo.id} className={selected.imageId === photo.id ? "active" : ""} title={photo.name} aria-label={`Use photo: ${photo.name}`} onClick={() => patchScene({ imageId: photo.id, template: "image" })}><img src={photo.image.src} alt={photo.name} /></button>)}</div><input hidden ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { if (event.target.files?.[0]) void upload(event.target.files[0]); event.target.value = ""; }} /><div className="cs-segment cs-composition">{[{ id: "image", label: "Photo" }, { id: "title", label: "Type" }, { id: "endcard", label: "Logo" }].map((choice) => <button key={choice.id} className={selected.template === choice.id ? "active" : ""} onClick={() => { if (editor.doc.designVersion !== 2) edit({ ...editor, doc: { ...editor.doc, designVersion: 2 } }, false); patchScene({ template: choice.id as MotionScene["template"], imageId: choice.id === "image" ? selected.imageId || photos[0]?.id || null : null }); }}>{choice.label}</button>)}</div><div className="cs-panel-title">Text position</div><div className="cs-positions">{[{ id: "top-left", label: "Upper left" }, { id: "center-left", label: "Middle left" }, { id: "center", label: "Center" }, { id: "bottom-left", label: "Lower left" }, { id: "bottom-center", label: "Lower center" }, { id: "bottom-right", label: "Lower right" }].map((position) => <button key={position.id} aria-pressed={(selected.position || "bottom-left") === position.id} className={(selected.position || "bottom-left") === position.id ? "active" : ""} onClick={() => patchScene({ position: position.id as MotionScene["position"] })}>{position.label}</button>)}</div>{editor.mode === "video" && <><div className="cs-panel-title">Motion</div><div className="cs-segment cs-motion-options">{[{ id: "stagger", label: "Reveal" }, { id: "rise", label: "Rise" }, { id: "fade", label: "Fade" }, { id: "scale", label: "Grow" }].map((motion) => <button key={motion.id} className={selected.animation === motion.id ? "active" : ""} onClick={() => { patchScene({ animation: motion.id as MotionScene["animation"] }); timeRef.current = editor.doc.scenes.slice(0, selectedIndex).reduce((sum, scene) => sum + scene.duration, 0); setPlaying(true); }}>{motion.label}</button>)}</div><label className="cs-range">Scene length <span>{selected.duration / 1000}s</span><input type="range" min={1500} max={12000} step={500} value={selected.duration} onChange={(event) => patchScene({ duration: Number(event.target.value) })} /></label></>}
      <details><summary>Fine-tune</summary>{selected.imageId && <><label className="cs-range">Photo shading<input type="range" min={0} max={1} step={.05} value={selected.shade ?? .5} onChange={(event) => patchScene({ shade: Number(event.target.value) })} /></label><label className="cs-range">Crop left / right<input type="range" min={0} max={1} step={.02} value={selected.focalX ?? .5} onChange={(event) => patchScene({ focalX: Number(event.target.value) })} /></label><label className="cs-range">Crop up / down<input type="range" min={0} max={1} step={.02} value={selected.focalY ?? .5} onChange={(event) => patchScene({ focalY: Number(event.target.value) })} /></label>{editor.mode === "video" && <label className="cs-check"><input type="checkbox" checked={selected.zoom !== false} onChange={(event) => patchScene({ zoom: event.target.checked })} />Slow photo movement</label>}</>}{editor.mode === "video" && <label>Between scenes<select value={selected.transition} onChange={(event) => patchScene({ transition: event.target.value as MotionScene["transition"] })}><option value="fade">Crossfade</option><option value="slide">Reveal</option><option value="cut">Cut</option></select></label>}</details>
